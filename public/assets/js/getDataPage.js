...
async function SelectFilter(page, dates = [], courts = []) {
...
async function GetTableData(page, DTO, classFilters) {
    ...
    return await page.evaluate(async (classFilters, DTO) => {
        ...
        for (let row of rows) {
            for (let key in classFilters) {
                const civilElement = await row.querySelector(`.${classFilters[key]}`);
                if (civilElement) {
...
async function GetDataInPage2(page) {
    ...
    for (let element of elements) {
        ...
        await delayFile.randomDelay();
        await element.click();
        // Подождем некоторое время перед следующим кликом, если нужно
        await delayFile.randomDelay(); // 500 мс (можно настроить)
    }
...
//const clickCoordinates = await page.evaluate(() => {
//         return new Promise((resolve) => {
//             document.addEventListener('click', (event) => {
//                 const coords = {
//                     x: event.clientX,  // Координата X относительно окна просмотра
//                     y: event.clientY   // Координата Y относительно окна просмотра
//                 };
//                 resolve(coords);  // Возвращаем координаты
//             });
//         });
//     });
//    console.log('Координаты клика:', clickCoordinates);


// b-chrono-item-header js-chrono-item-header page-break b-chrono-item-header-expanded
//
// b-chrono-item js-chrono-item                         b-chrono-cols page-break g-ec
//
// b-chrono-item js-chrono-item                         b-chrono-cols page-break g-ec
//
// b-chrono-item js-chrono-item                         b-chrono-cols page-break g-ec even
//
//
// b-chrono-item-header js-chrono-item-header page-break b-chrono-item-header-expanded
//
// container container--live_translation
//
// b-chrono-item js-chrono-item                         b-chrono-cols page-break g-ec
//
// b-chrono-item js-chrono-item                         b-chrono-cols page-break g-ec even
